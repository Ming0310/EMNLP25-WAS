...
def _new_attn_forward(self, x: Tensor, freqs_cis: Tensor, mask: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
    ...
    q,k,v = self.gemv1(x, self.wqkv.weight, self.thresh_q, self.thresh_k, self.thresh_v, self.q_norm, self.k_norm, self.v_norm, self.sparsity_bin, kv_size).split([self.dim, kv_size, kv_size], dim=-1)
    ...
    y = self.gemv2(y, self.wo.weight, self.wo_norm, self.thresh_o, self.sparsity_bin) # prefill logic taken care of in gemv
...
def _new_ffn_forward(self, x: Tensor) -> Tensor:
    gate_x = F.silu(self.gemv1(x, self.w1.weight,self.w1_norm, self.thresh_gate, self.sparsity_bin))
    return self.gemv2(gate_x * self.gemv1(x, self.w3.weight, self.w3_norm, self.thresh_up, self.sparsity_bin), self.w2.weight, self.w2_norm, self.thresh_down, self.sparsity_bin) 
...
class FeedForward(nn.Module):
    def __init__(self, config: ModelArgs) -> None:
        ...
        self.w1_norm = torch.ones(1, 1, config.dim)
        self.w2_norm = torch.ones(1, 1, config.intermediate_size)
        self.w3_norm = torch.ones(1, 1, config.dim)
        self.sparsify = False
