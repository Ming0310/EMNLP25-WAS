...
def reverse_permute(w, n_head, config):
    dim = config.dim
    return (
        w.view(n_head, config.head_dim // 2, 2, dim)  
        .transpose(1, 2)
        .reshape(n_head * config.head_dim, dim) 
    )

def get_norm(wqkv, config):
    total_dim = wqkv.shape[0]  
    q_dim = config.n_head * config.head_dim
    k_dim = config.n_local_heads * config.head_dim
    v_dim = config.n_local_heads * config.head_dim 

    q, k, v = torch.split(wqkv, [q_dim, k_dim, v_dim], dim=0)


    q = reverse_permute(q, config.n_head, config)
    k = reverse_permute(k, config.n_local_heads, config)
    q = torch.norm(q, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
    k = torch.norm(k, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
    v = torch.norm(v, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
    return q, k, v
...
def _load_model(checkpoint_path, device, precision, use_tp, hist_path, sparsity):
    ...
    def monkeypatch_layer(layer_idx, layer, sparsity, hist_path, device):
        ...
        distrs["mlp_gate"] = Distribution(mlp_path, "gate")
        distrs["mlp_up"] = Distribution(mlp_path, "up")
        distrs["mlp_down"] = Distribution(mlp_path, "down")
        distrs["attn_q"] = Distribution(attn_path, "q")
        distrs["attn_k"] = Distribution(attn_path, "k")
        distrs["attn_v"] = Distribution(attn_path, "v")
        distrs["attn_o"] = Distribution(attn_path, "o")

        sparses = {}
        for proj in projs:
            val = 0.5 + 0.5*sparsities[proj][layer_idx]
            if proj in ['q', 'k', 'v']:
                sparses[proj] = distrs[f"attn_{proj}"].icdf(val).item()
            elif proj in ['o']:
                sparses[proj] = distrs[f"attn_{proj}"].icdf(val).item()
            elif proj in ['up', 'gate']:
                sparses[proj] = distrs[f"mlp_{proj}"].icdf(val).item()
            elif proj in ['down']:
                sparses[proj] = distrs[f"mlp_{proj}"].icdf(val).item()
        ...
        layer.feed_forward.w1_norm = torch.norm(layer.feed_forward.w1.weight.data, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
        layer.feed_forward.w2_norm = torch.norm(layer.feed_forward.w2.weight.data, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
        layer.feed_forward.w3_norm = torch.norm(layer.feed_forward.w3.weight.data, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
        # layer.feed_forward.w1.weight.data = layer.feed_forward.w1.weight.data / layer.feed_forward.w1_norm.squeeze(0).squeeze(0)
        # layer.feed_forward.w3.weight.data = layer.feed_forward.w3.weight.data / layer.feed_forward.w3_norm.squeeze(0).squeeze(0)
        ...
        layer.feed_forward.sparsity_bin = 0
        # layer.feed_forward.w2.weight.data = layer.feed_forward.w2.weight.data / layer.feed_forward.w2_norm.squeeze(0).squeeze(0)
        layer.feed_forward.w2.weight.data = layer.feed_forward.w2.weight.data.T.contiguous().T # column major

        is_sparse = True
        q, k, v = get_norm(layer.attention.wqkv.weight, config)
        layer.attention.q_norm = q
        layer.attention.k_norm = k
        layer.attention.v_norm = v
        ...
        layer.attention.gemv1 = layer.attention.gemv1_kernel.operator(is_sparse)
        ...
        layer.attention.wo_norm = torch.norm(layer.attention.wo.weight.data, dim=0, p=1).unsqueeze(0).unsqueeze(0).contiguous()
        # layer.attention.wo.weight.data = layer.attention.wo.weight.data / layer.attention.wo_norm.squeeze(0).squeeze(0)
        ...
        layer.attention.gemv2 = layer.attention.gemv2_kernel.operator(is_sparse)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lookup_dir', type=str, default=None, help="lookup_dir")
