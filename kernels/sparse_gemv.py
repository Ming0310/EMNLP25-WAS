...
def splitk_sparse_gemv_kernel(
    ...
    if BATCHSIZE == 1:
        ...
        norm_val = tl.load(weight_norm + rm, mask=rm < M, other=1.0)
        scaled_x0 = x0 * norm_val
        idx = tl.abs(scaled_x0) > threshold
        # selectively load weight rows
        valid_mask = (rm[:, None] < M) & (rn[None, :] < N) & idx[:, None]
        a = tl.load(A_ptr, mask=valid_mask, other=0.0, eviction_policy='evict_first') # only load weights once per threadblock
...
def qkv_kernel(
    ...
    scale_q_ptr, scale_k_ptr, scale_v_ptr,
    # Matrix dimensions
    N, N_q, N_kv, M,
    CACHE_KEY_N, CACHE_KEY_M,
    # Meta-parameters
    ...
    scale_ptr = tl.where(is_q, scale_q_ptr, tl.where(is_v, scale_v_ptr, scale_k_ptr))
    scale = tl.load(scale_ptr + rm, mask=rm < M, other=1.0)
    if BATCHSIZE == 1:
...
def qkv_gemv(
    ...
    threshold_k: float,
    threshold_v: float,
    scale_q_ptr: torch.Tensor, 
    scale_k_ptr: torch.Tensor, 
    scale_v_ptr: torch.Tensor,
    sparsity_bin: int,
    kv_size: int
):
    N, Z = weight.shape
...
class SparseGEMV(BaseKernel):
    ...
    def forward(
        ...
        return splitk_sparse_gemv(hidden_states, weights, weight_norm, threshold, sparsity_bin) if hidden_states.shape[1] == 1 else torch.matmul(hidden_states, weights.T) 
...
class SparseQKVGEMV(BaseKernel):
    ...
    def forward(
        ...
        return qkv_gemv(x, weight, threshold_q, threshold_k, threshold_v, scale_q_ptr, scale_k_ptr, scale_v_ptr, sparsity_bin, kv_size) if x.shape[1] == 1 else torch.matmul(x, weight.T)
