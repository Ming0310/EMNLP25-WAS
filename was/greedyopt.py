...
from was.model import LlamaSparseForCausalLM, LlamaSparseConfig, MistralSparseForCausalLM, MistralSparseConfig
...
def process_layer(layer, model_type, layer_idx, target_sparsity, base_step_size, last_fraction, was_path):
    weights = weight_dict[model_type]

    activations_path = os.path.join(was_path, 'activations', f'act_{layer_idx}.pt')
    output_path = os.path.join(was_path, 'lookup', f'layer-{layer_idx}', 'results.csv')
...
if __name__ == "__main__":
    ...
    process_layer(layer, args.model_type, layer_idx, args.target_sparsity, args.base_step_size, args.last_fraction, args.was_path)
